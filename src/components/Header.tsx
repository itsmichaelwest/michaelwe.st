  ...
  render() {
    ...
    return (
      <header className={!this.state.shown ? `${classes}` : `bg-white ${classes}`}>
        <a className="flex items-center text-xl font-header font-semibold tracking-tight text-gray-900 hover:text-blue pointer-events-auto transition-colors" href="/" aria-label={this.props.siteTitle}>
        ...
        <div className="flex-grow pointer-events-auto">
            ...
            <nav className="flex-auto">
              {this.props.menuLinks.map(link => (
                <Link 
                  key={link.link}
                  to={link.link}
                  className="font-header ml-8 text-gray-400 text-opacity-70 hover:text-blue hover:text-opacity-100 transition-all"
          ...
          <div className="float-right -my-1 lg:hidden">
            <button type="button" style={{ backdropFilter: 'blur(30px)' }} className="bg-white bg-opacity-10 border border-gray-100 rounded-md p-2 inline-flex items-center justify-center text-gray-400 text-opacity-80 hover:text-gray-500 hover:text-opacity-100 hover:bg-gray-50 hover:bg-opacity-90 outline-none focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue transition-all hover:shadow" onClick={this.toggleHidden.bind(this)}>
              {!this.state.shown
              ?
              <div>
        ...
        {this.state.shown &&
        <div className="absolute top-24 sm:top-32 inset-0 h-screen transition-all bg-white pointer-events-auto">
          ...
          {this.props.menuLinks.map(link => (
            <div key={link.link}>
              <Link 
                  key={link.link}
                  to={link.link}
                  className="block w-100 h-full px-8 sm:px-16 py-4 sm:py-4 font-header text-black hover:bg-blue-700 hover:text-white"
